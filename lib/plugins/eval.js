...
module.exports = {
  ...
  methods: {
    ...
    expression(val, name) {
      const self = this;
      const parents = self.getParentsName(name).map(parent => self.getData(parent));
      const ctx = _.extend({}, ...parents, self.getData(), {
        _: _,
        _name: name,
        self: self,
        root: self.rootData(),
      ...
      try {
        ...
        console.log(`${name}="${val}": ${err.message}`); // (keys: ${_.keys(ctx).join(', ')})
      }
      return res;
    },
